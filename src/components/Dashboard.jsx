...
const Dashboard = ({ currentUser, onLogout }) => {
  ...
  const loadData = async () => {
    try {
      // Updated to match your new serverless function parameter name
  ...
  const volumeData = {
    ...
    datasets: [{
      label: 'Volume',
      data: workouts.slice(0, 7).map(w => (Number(w.weight) || 0) * (Number(w.reps) || 0) * (Number(w.sets) || 1)).reverse(),
  ...
  const donutData = {
    ...
    datasets: [{
      data: workouts.reduce((acc, w) => {
        ...
        const idx = acc.labels.indexOf(muscle);
        if (idx > -1) acc.data[idx]++;
        else { acc.labels.push(muscle); acc.data.push(1); }
        return acc;
      }, { labels: [], data: [] }).data,
      backgroundColor: ['#38bdf8', '#818cf8', '#fbbf24', '#f87171', '#34d399'],
    }]
  };
    ...
    <div style={theme.wrapper}>
      ...
      <div style={theme.dashboardGrid}>
        <div style={theme.column}>
          ...
          <div style={{...theme.glassCard, marginTop: '24px'}}>
             <h3 style={theme.cardTitle}>Muscle Split</h3>
        ...
        <div style={theme.column}>
          <div style={theme.glassCard}>
            ...
            {workouts.slice(0, 8).map((w, i) => (
              <div key={w.id || i} style={theme.activityItem}>
                <div>
                  <div style={{fontWeight: 'bold'}}>{w.exercise}</div>
                  <div style={{fontSize: '10px', color: '#64748b'}}>{new Date(w.created_at).toLocaleDateString()}</div>
                </div>
                <span style={{ color: '#38bdf8' }}>{w.weight}{ACTIVITY_CONFIG[w.exercise]?.label3 || 'kg'}</span>
              </div>
            ))}
          </div>
        </div>
...
const theme = {
  ...
  avatar: { width: '50px', height: '50px', borderRadius: '50%', background: '#38bdf8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' },
  badge: { fontSize: '12px', color: '#38bdf8' },
  dashboardGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' },
  glassCard: { background: 'rgba(30, 41, 59, 0.7)', borderRadius: '24px', padding: '24px', border: '1px solid rgba(255,255,255,0.05)' },
  actionCard: { background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', borderRadius: '24px', padding: '24px', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' },
  cardTitle: { fontSize: '16px', color: '#94a3b8', marginBottom: '20px', textTransform: 'uppercase', letterSpacing: '1px' },
  ...
  label: { fontSize: '10px', color: 'rgba(255,255,255,0.7)', marginBottom: '5px', display: 'block' },
  row: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' },
  primaryBtn: { width: '100%', padding: '14px', borderRadius: '12px', border: 'none', backgroundColor: '#fff', color: '#4f46e5', fontWeight: 'bold', cursor: 'pointer', transition: 'transform 0.2s' },
  activityItem: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' },
  logoutBtn: { background: 'transparent', color: '#ef4444', border: '1px solid #ef4444', borderRadius: '8px', padding: '8px 16px', cursor: 'pointer' }
};
